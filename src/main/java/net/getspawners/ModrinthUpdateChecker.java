...
public final class ModrinthUpdateChecker {
    ...
    private static void checkForUpdate() {
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("User-Agent", "GetSpawners/" + currentVersion())
                .GET()
                .build();
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                GetSpawnersMod.LOGGER.debug("[GetSpawners] Update check returned HTTP {}.", response.statusCode());
                return;
            }

            Optional<String> latestVersion = extractLatestVersion(response.body());
            if (latestVersion.isEmpty()) {
                GetSpawnersMod.LOGGER.debug("[GetSpawners] Update check returned no usable versions.");
                return;
            }
            ...
            if (isNewerVersion(newestVersion, currentVersion)) {
                GetSpawnersMod.LOGGER.debug("[GetSpawners] New version available: {} (current: {})",
            ...
            GetSpawnersMod.LOGGER.debug("[GetSpawners] Update check failed.", e);
        }
    }
